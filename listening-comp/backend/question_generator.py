...
class QuestionGenerator:
    def __init__(self, api_key: str):
        """Initialize Mistral client and vector store"""
        self.mistral_client = mistral.Client(api_key=api_key)
        self.vector_store = QuestionVectorStore()

    def _invoke_mistral(self, prompt: str) -> Optional[str]:
        """Invoke Mistral with the given prompt"""
        try:
            response = self.mistral_client.generate_text(prompt, temperature=0.7)
            return response['text']
        except Exception as e:
            print(f"Error invoking Mistral: {str(e)}")
            return None
    ...
    def get_feedback(self, question: Dict, selected_answer: int) -> Dict:
        ...
        prompt = f"""Given this French listening question and the selected answer, provide feedback explaining if it's correct 
